...
public abstract class SessionFactoryHandler {
	...
	private void onUnableToConnect(Exception e) {
		Platform.runLater(() -> {
			/*
			 * String contentText =
			 * "Le logiciel n'a pas pu se connecter au serveur, " +
			 * "soit l'hôte de déstination est injoignable, " +
			 * "soit MySQL n'est pas installé, " +
			 * "soit la configuration a été mal faite !";
			 */

			Alert alert = new Alert(AlertType.ERROR);
			alert.setTitle(this.applicationName);
			alert.setHeaderText("Erreur de connexion au serveur");
