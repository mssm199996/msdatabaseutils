...
public interface IDAO<T> extends ISimpleDAO {
    ...
    public default T findById(Class<T> entityClass, int id) {
        return this.getSessionFactoryHandler().findById(entityClass, id);
    }
    ...
    public default String getMultipleElementsUpdateReport(T... elements) {
        return "MISE A JOUR DE " + elements.length + " " + this.getNameOfEntity();
    }
    ...
    public default String getMultipleElementsUpdateReport(Collection<T> elements) {
        return "MISE A JOUR DE " + elements.size() + " " + this.getNameOfEntity();
    }
